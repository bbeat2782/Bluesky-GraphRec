...
class TGAT(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, num_layers: int = 2, dropout: float = 0.1, device: str = 'cpu'):
        """
        TGAT without Multi-Head Attention (MHA).
        """
        ...
        # Replace Multi-Head Attention with an MLP or Mean Pooling Layer
        self.temporal_conv_layers = nn.ModuleList([
            nn.Linear(self.node_feat_dim + self.edge_feat_dim + self.time_feat_dim, self.node_feat_dim)
            for _ in range(num_layers)
        ])

        # Merge layer to combine output with raw node features
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        """
        Compute node embeddings **without Multi-Head Attention**
        """
        ...
        if current_layer_num == 0:
            ...
            neighbor_node_features = self.compute_node_temporal_embeddings(
                node_ids=neighbor_node_ids.flatten(),
                node_interact_times=neighbor_times.flatten(),
                current_layer_num=current_layer_num - 1,
                num_neighbors=num_neighbors
            ).reshape(node_ids.shape[0], num_neighbors, self.node_feat_dim)

            # Compute mean of neighbor embeddings (Mean Aggregation instead of Attention)
            neighbor_agg_features = neighbor_node_features.mean(dim=1)
            ...
            # Combine features (Node + Time + Edge)
            combined_features = torch.cat([neighbor_agg_features, neighbor_time_features.mean(dim=1), neighbor_edge_features.mean(dim=1)], dim=-1)

            # Pass through linear layer (instead of Multi-Head Attention)
            output = self.temporal_conv_layers[current_layer_num - 1](combined_features)

            # Merge with raw node features
