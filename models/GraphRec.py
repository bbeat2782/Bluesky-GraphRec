...
class GraphRec(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        # three lists to store source nodes' second-hop neighbor ids, edge ids and interaction timestamp information, with batch_size as the list length
        src_nodes_neighbor_ids_list, src_nodes_edge_ids_list, src_nodes_neighbor_times_list = \
            self.neighbor_sampler.get_multi_hop_neighbors(num_hops=2, node_ids=src_node_ids, node_interact_times=node_interact_times, num_neighbors=10)

        # three lists to store destination nodes' second-hop neighbor ids, edge ids and interaction timestamp information, with batch_size as the list length
        dst_nodes_neighbor_ids_list, dst_nodes_edge_ids_list, dst_nodes_neighbor_times_list = \
            self.neighbor_sampler.get_multi_hop_neighbors(num_hops=2, node_ids=dst_node_ids, node_interact_times=node_interact_times, num_neighbors=10)

        # TODO need to change how to use multi-hop neighbors
        src_padded_nodes_neighbor_ids = np.concatenate((src_nodes_neighbor_ids_list[0], src_nodes_neighbor_ids_list[1]), axis=1)
        src_padded_nodes_edge_ids = np.concatenate((src_nodes_edge_ids_list[0], src_nodes_edge_ids_list[1]), axis=1)
        src_padded_nodes_neighbor_times = np.concatenate((src_nodes_neighbor_times_list[0], src_nodes_neighbor_times_list[1]), axis=1)
        dst_padded_nodes_neighbor_ids = np.concatenate((dst_nodes_neighbor_ids_list[0], dst_nodes_neighbor_ids_list[1]), axis=1)
        dst_padded_nodes_edge_ids = np.concatenate((dst_nodes_edge_ids_list[0], dst_nodes_edge_ids_list[1]), axis=1)
        dst_padded_nodes_neighbor_times = np.concatenate((dst_nodes_neighbor_times_list[0], dst_nodes_neighbor_times_list[1]), axis=1)        
        ...
        #     self.pad_sequences(node_ids=src_node_ids, node_interact_times=node_interact_times, nodes_neighbor_ids_list=src_nodes_neighbor_ids_list_one,
        #                        nodes_edge_ids_list=src_nodes_edge_ids_list_one, nodes_neighbor_times_list=src_nodes_neighbor_times_list_one,
        ...
        # print(src_padded_nodes_neighbor_ids[0])

        # print(dst_padded_nodes_neighbor_ids[0])
        # print(len(src_padded_nodes_neighbor_ids))
        # raise ValueError()
