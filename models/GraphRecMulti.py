...
class GraphRecMulti(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        """
        super(GraphRecMulti, self).__init__()
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, batch_src_idx=None, is_eval=False):
        ...
        # # ======================== SRC NODES ========================
        # src_padded_nodes_neighbor_ids = np.array([
        #     np.concatenate([first_hop, second_hop])  # Already (8 + 64) = 72
        #     for first_hop, second_hop in zip(src_nodes_neighbor_ids_list[0], src_nodes_neighbor_ids_list[1])
        # ], dtype=np.int32)
        
        # src_padded_nodes_edge_ids = np.array([
        #     np.concatenate([first_hop, second_hop])
        #     for first_hop, second_hop in zip(src_nodes_edge_ids_list[0], src_nodes_edge_ids_list[1])
        # ], dtype=np.int32)
        
        # src_padded_nodes_neighbor_times = np.array([
        #     np.concatenate([first_hop, second_hop])
        #     for first_hop, second_hop in zip(src_nodes_neighbor_times_list[0], src_nodes_neighbor_times_list[1])
        # ], dtype=np.float32)
        
        # # ======================== DST NODES ========================
        # dst_padded_nodes_neighbor_ids = np.array([
        #     np.concatenate([first_hop, second_hop])  # Already (8 + 64) = 72
        #     for first_hop, second_hop in zip(dst_nodes_neighbor_ids_list[0], dst_nodes_neighbor_ids_list[1])
        # ], dtype=np.int32)
        
        # dst_padded_nodes_edge_ids = np.array([
        #     np.concatenate([first_hop, second_hop])
        #     for first_hop, second_hop in zip(dst_nodes_edge_ids_list[0], dst_nodes_edge_ids_list[1])
        # ], dtype=np.int32)
        
        # dst_padded_nodes_neighbor_times = np.array([
        #     np.concatenate([first_hop, second_hop])
        #     for first_hop, second_hop in zip(dst_nodes_neighbor_times_list[0], dst_nodes_neighbor_times_list[1])
        # ], dtype=np.float32)
        ...
        #                        patch_size=self.patch_size, max_input_sequence_length=self.max_input_sequence_length, batch_src_idx=batch_src_idx)
