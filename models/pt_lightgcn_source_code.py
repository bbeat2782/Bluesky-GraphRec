...
class LightGCN(torch.nn.Module):
    ...
    **recommendation** (via
    ...
    def get_embedding(
        self,
        edge_index: Adj,  # Shape [2, num_edges] - Source and target node indices
        edge_weight: OptTensor = None,  # Shape [num_edges] if provided - Weight of each edge
    ) -> Tensor:
        r"""Returns the embedding of nodes in the graph.
        
        The returned embedding tensor has shape [num_nodes, embedding_dim] and contains
        the final node representations after message passing."""
        x = self.embedding.weight  # Shape [num_nodes, embedding_dim]
        out = x * self.alpha[0]  # Shape [num_nodes, embedding_dim]

        for i in range(self.num_layers):
            x = self.convs[i](x, edge_index, edge_weight)  # Shape [num_nodes, embedding_dim]
            out = out + x * self.alpha[i + 1]  # Shape [num_nodes, embedding_dim]

        return out  # Shape [num_nodes, embedding_dim]
